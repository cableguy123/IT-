...
- 정규 표현식에서 유니코드 문자 속성을 나타내는 문자 표현

> \p{Is<속성명>}

- \p{Is<속성명>}:
특정 유니코드 블록이나 속성을 가진 문자를 찾습니다. 예를 들어, `\p{IsGreek}`는 그리스 문자, `\p{IsLatin}`은 라틴 문자를 찾습니다.

- \p{java<속성명>}:
자바의 유니코드 문자 속성을 사용함 `\p{javaUpperCase}`는 자바의 대문자 속성을 가진 문자를 찾음 

- \p{gc=<그룹명>}:
`\p{gc=Lu}`는 대문자, `\p{gc=Ll}`는 소문자, `\p{gc=Nd}`는 숫자를 찾습니다
