...
Model / View / ViewModel로 나누어진 디자인 패턴
뷰 로직,비즈니스 로직을 분리하는것이 목표
ViewModel 에서는 View를 위한 상태와 메서드를 정의하고 View는 ViewModel의 상태변화를 "옵저버 패턴"을 통해 관찰함
-> 이를 데이터 바인딩이라고 표현
ViewModel 은 View가 쉽게 사용할수있도록 Model의 데이터를 가공해서 View에게 전달
Model은 데이터와 비즈니스 로직을 포함(DB처리,네트워크 API에서 데이터를 가져오고 처리 및 저장하는 책임을 가짐)
View : ViewModel = N : 1

장점
- 뷰 로직과 비즈니스 로직을 분리했기때문에 테스트 및 유지 보수가 용이
- 개발자와 디자이너가 병렬적으로 작업이 가능,UI 디자인이 나오지않았더라도 미리 정의된 뷰와 뷰 모델을 먼저 개발
- 코드 변경을 최소화하며 수정 가능

```

# Observer 패턴 (2025/06/04) 
```
객체 간의 의존성을을 줄이고,객체 간의 상호작용을 느슨하게 만들기위한 디자인 패턴 중 하나
어떤 객체의 상태가 변화를 할때 그를 관찰하는 구독자들에게 이벤트를 발생시켜주는 디자인 패턴
이벤트를 발행하는 객체가 Publisher , 구독자를 Observer
이 패턴 사용함으로써 객체지향의 Open-Close원칙을 지킬수있음 
```
